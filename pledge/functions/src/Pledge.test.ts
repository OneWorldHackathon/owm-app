...
describe('Test Pledge entity', () => {
  ...
  it('test uuid must be a valid uuid', () => {
    const pledge = Pledge.newInstance(VALID_USER_ID, VALID_DISTANCE_M, VALID_LOCATION)
  ...
  it('test distance cannot exceed 26.2 miles', () => {
    expect(() => {
      Pledge.newInstance(VALID_USER_ID, Conversions.milesToMetres(26.3), VALID_LOCATION)
    }).to.throw(ValidationException)
  })
  it('test distance cannot be less than 100 metres', () => {
    expect(() => {
      Pledge.newInstance(VALID_USER_ID, 99, VALID_LOCATION)
    }).to.throw(ValidationException)
  })
